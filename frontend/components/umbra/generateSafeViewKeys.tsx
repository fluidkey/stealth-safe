...
const umbra = new Umbra(provider, 100);

// modification of Umbra's generatePrivateKeys function
export async function generateSafePrivateKeys(signer: Signer){
    ...
    const { chainId } = await umbra.provider.getNetwork();
    ...
    const signature = String(await umbra.provider.send('personal_sign', [formattedMessage, userAddress.toLowerCase()]));
